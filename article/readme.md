...
Чтобы избежать потери ресурсов в цикле, посмотрим как можно блокировать поток или усыпить. Все структуры, которые используют ядро системы часто медленнее, чем те, что в пространстве пользователя. Медленее в несколько раз, например AutoResetEvent может быть в 53 раза медленнее SpinLock [Рихтер]. Но с их помощью можно синхронизировать процессы по всей системе, управляемые или нет.

Основная конструкция здесь это семафор, предложенный Дейкстрой более полувека назад. Семафор это, упрощенно говоря, положительное целое число, управляемое системой, и две операции на этом числе, увеличить и уменьшить. Если уменьшить не получаеться, оно ноль, то вызывающий поток блокируется. Когда число увеличивается кем-нибудь, тогда потоки пропускаются и число уменьшается. Можно представить поезда в узком месте с семафором. .NET предлагает несколько конструкций с подобными функциями: AutoResetEvent, ManualResetEvent, Mutex и сам Semaphore. Мы будем использовать AutoResetEvent, это самая простая из этих конструкций: только два значения 0 и 1 (false, true). Ее метод WaitOne() блокирует вызывающий поток, если значение было 0, а если 1, то понижает до 0 и возращает. А метод Set() выставляет значение 1. Действует, как турникет в метро.
